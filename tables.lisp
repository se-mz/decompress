...
;;; https://www.rfc-editor.org/rfc/rfc6234#page-11
(declaim (type (simple-array (unsigned-byte 32) (64)) +sha-256-constants+))
(define-constant +sha-256-constants+
    (let ((src "
...
"))
      (map '(simple-array (unsigned-byte 32) (*))
           (lambda (start)
             (parse-integer src :radix 16 :start start :end (+ start 8)))
           (remove-if-not (lambda (i)
                            (char= #\Space (char src (- i 1))))
                          (positions-if #'hexdigitp src))))
  :test 'equalp)

;;; https://www.rfc-editor.org/rfc/rfc6234#page-13
(declaim (type (simple-array (unsigned-byte 32) (8)) +sha-256-init+))
(define-constant +sha-256-init+
    (let ((src "
      H(0)0 = 6a09e667
      H(0)1 = bb67ae85
      H(0)2 = 3c6ef372
...
"))
      (map '(simple-array (unsigned-byte 32) (*))
           (lambda (start)
             (parse-integer src :radix 16 :start (+ start 2) :end (+ start 2 8)))
           (positions #\= src)))
  :test 'equalp)

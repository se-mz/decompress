...
(defun basic-update-crc-32 (data start end state)
  (declare (type buffer data)
           (type array-length start end)
           (type (unsigned-byte 32) state)
           (optimize speed))
  (assert (<= 0 start end (length data)))
  ;; Safety 0 makes a crazy difference under ECL (2.8x) and improves CCL a bit
  ;; as well (1.16x).
  (locally (declare (optimize (safety 0)))
    (loop :while (< start end) :do
      (setf state (logxor (aref +crc-32-table+ (logand #xFF (logxor state (aref data start))))
                          (ash state -8)))
      (incf start)))
  state)
...
(onetime-macrolet ((var-count 2)
  ...
  (dotimes (i 256)
    (setf (aref table 0 i) (aref +crc-32-table+ i)))
  (dotimes (i 256)
  ...
  `(defun fast-update-crc-32 (data start end state)
     (declare (type buffer data)
              (type array-length start end)
              (type (unsigned-byte 32) state)
              (optimize speed))
     ...
     (basic-update-crc-32 data start end state)))

...
|                          | semz.decompress-1.1.0            | chipz-20230408                                        | deflate-1.0.1 (Mai)                       |
|--------------------------+----------------------------------+-------------------------------------------------------+-------------------------------------------|
| /                        | <                                |                                                       |                                           |
| supported formats        | deflate, zlib, gzip, bzip2       | deflate, zlib, gzip, bzip2                            | deflate, zlib, gzip                       |
...
| edge case                                                      | zlib (zpipe) | semz         | Mai                     | chipz          |
|----------------------------------------------------------------+--------------+--------------+-------------------------+----------------|
| /                                                              | <            |              |                         |                |
| reserved block type                                            | error        | error        | error                   | error          |
| uncompressed zero-length block                                 | handled      | handled      | handled                 | handled        |
| uncompressed block with wrong checksum                         | error        | error        | error                   | error          |
| uncompressed zero-length block with wrong checksum             | error        | error        | error                   | outputs data   |
| reference goes beyond previous output                          | error        | error        | outputs data            | outputs data   |
| uses illegal length code 286/287                               | error        | error        | error                   | internal error |
| uses illegal distance code 30/31                               | error        | error        | longer range than 29    | internal error |
|----------------------------------------------------------------+--------------+--------------+-------------------------+----------------|
| dynamic block provides encodings for 286/287 & 30/31           | error        | outputs data | outputs data            | outputs data   |
| dynamic block with one distance code (§3.2.7)                  | handled      | handled      | handled                 | handled        |
| dynamic block with one distance code, uses the unassigned code | error        | error        | error                   | internal error |
| dynamic block without distance codes (§3.2.7)                  | handled      | handled      | handled                 | handled        |
| dynamic block without distance codes, tries to use len code    | error        | error        | error                   | internal error |
| Huffman tree with too many items                               | error        | error        | outputs data            | outputs data   |
| Huffman tree with too few items                                | error        | error        | outputs data            | outputs data   |
| Huffman tree with too few items, uses an unassigned code       | n/a          | n/a          | error                   | internal error |
| dynamic block has code lengths expand across len/dist boundary | handled      | handled      | handled                 | basic error    |
| dynamic block has code lengths expand out of bounds            | error        | error        | internal error          | basic error    |
| dynamic block starts by repeating the last code length         | error        | error        | internal error          | internal error |
|----------------------------------------------------------------+--------------+--------------+-------------------------+----------------|
| reference stays in bounds but goes beyond zlib window size     | outputs data | error        | outputs data            | outputs data   |
| zlib preset dictionary required but none provided              | error        | error        | error                   | outputs data¹  |
| wrong Adler-32 checksum                                        | error        | error        | outputs data by default | error          |
|----------------------------------------------------------------+--------------+--------------+-------------------------+----------------|
| wrong gzip magic bytes                                         | error        | error        | error                   | error          |
| gzip data with wrong CRC-32 checksum                           | error        | error        | outputs data by default | error          |
| gzip data with right CRC-32 checksum but wrong modular length  | error        | error        | outputs data            | outputs data   |
| gzip data with header checksum                                 | handled      | handled      | handled                 | handled        |
| gzip data with incorrect header checksum                       | error        | error        | outputs data            | outputs data   |
| gzip extra fields                                              | handled      | handled      | ignored                 | basic error    |
| inconsistent length fields in extra fields section             | outputs data | error        | outputs data            | n/a            |
